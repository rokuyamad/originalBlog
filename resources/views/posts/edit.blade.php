...
@section('content')
                  ...
                  <ul class="category-list">
                    @for ($i = 0; $i < $categories->count(); $i++)
                      ...
                      @if ($post->category_id == $categories[$i]->id)
                        {{Form::radio('category_id', $categories[$i]->id, true, ['class' => 'radio-btn', 'id' => "radio{$i}"])}}
                      @else
  ...
  <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCe-ZCBPyFsLINc3_1XaiENnevJ7M6ebaQ"></script>
  <script>
    $(function() {
      $('#tags').tagsInput({
        height: '203px',
      ...
      // script only for edit.blade.php
      ...
      initMap(pointArray);
      // end scripts for only edit.blade.php
    }); 
